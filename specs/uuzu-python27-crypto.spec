%if 0%{?rhel} && 0%{?rhel} <= 6
%{!?__python2: %global __python2 /data/app/python/bin/python2.7}
%{!?python2_sitelib: %global python2_sitelib %(%{__python2} -c "from distutils.sysconfig import get_python_lib; print(get_python_lib())")}
%{!?python2_sitearch: %global python2_sitearch %(%{__python2} -c "from distutils.sysconfig import get_python_lib; print(get_python_lib(1))")}
%endif

# Python3 introduced in Fedora 13.  This package is only destined for EPEL6
%global with_python3 0

Summary:	Cryptography library for Python
Name:		uuzu-python27-crypto
Version:	2.6.1
Release:	1%{?dist}
...
# Use setuptools to build so that we can create an egg
Patch100:	pycrypto-setuptools-for-compat-package.patch
Provides:	pycrypto = %{version}-%{release}
BuildRequires:	uuzu-python27-devel >= 2.2
...
%{?filter_provides_in: %filter_provides_in %{python2_sitearch}/Crypto/.*\.so}
%if %{with_python3}
...
This package provides the 2.6.x version of PyCrypto

%if %{with_python3}
%package -n python3-crypto2.6
Summary:	Cryptography library for Python 3
Group:		Development/Libraries

%description -n python3-crypto2.6
PyCrypto is a collection of both secure hash functions (such as MD5 and
SHA), and various encryption algorithms (AES, DES, RSA, ElGamal, etc.).
...
CFLAGS="%{optflags} -fno-strict-aliasing" %{__python2} setup.py bdist_egg

%if %{with_python3}
cd %{py3dir}
CFLAGS="%{optflags} -fno-strict-aliasing" %{__python3} setup.py bdist_egg
cd -
%endif
...
/data/app/python/bin/easy_install-2.7 -m --prefix %{buildroot}/data/app -Z dist/*.egg

# easy_install doesn't understand platlib (aka sitearch)
#if [ x'%{_lib}' = x'lib64' ] ; then
...
# Remove execute from python module files
find %{buildroot}%{python2_sitearch} -name '*.py' -type f -exec chmod 0644 {} \;
...
easy_install-%{python3_version} -m --prefix %{buildroot}%{_usr} -Z dist/*.egg
...
# Remove execute from python module files
find %{buildroot}%{python3_sitearch} -name '*.py' -type f -exec chmod 0644 {} \;
%endif
...
%{python2_sitearch}/*egg*/

%if %{with_python3}
%files -n python3-crypto2.6
%defattr(-,root,root,-)
%{python3_sitearch}/*egg*/
%endif

%changelog
* Fri Apr 11 2014 Harrison Zhu <zhuhuipeng@cyou-inc.com> - 2.6.1-1
- remove doc build with python 2.7
* Mon Mar 10 2014 Toshio Kuratomi <toshio@fedoraproject.org> - 2.6.1-1
- First build of python-crypto2.6
