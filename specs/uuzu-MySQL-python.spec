...
Name: uuzu-MySQL-python
Version: 1.2.3
Release: 0.3.c1.2%{?dist}
License: GPLv2+
...
Requires: uuzu-mysql-libs

BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}-root
#BuildRequires: python-setuptools
BuildRequires: uuzu-python27-devel
BuildRequires: uuzu-mysql-devel zlib-devel
...
%prep
%setup -q -n %{real_name}-%{version}c1

%patch2 -p1

%build
echo "mysql_config='%{install_root}/mysql/bin/mysql_config'" >> site.cfg
rm -f doc/*~
...
%defattr(-,root,root)
#%doc README GPL doc/*
%dir %{lib_path}/python?.?/site-packages/MySQLdb
%exclude %{lib_path}/python?.?/site-packages/MySQLdb/*.pyc
%exclude %{lib_path}/python?.?/site-packages/MySQLdb/constants/*.pyc
%exclude %{lib_path}/python?.?/site-packages/*.pyc
#%dir /usr/%{_lib}/python?.?/site-packages/MySQLdb/constants

%changelog
* Mon Nov 23 2009 Dennis Gregorovic <dgregor@redhat.com> - 1.2.3-0.3.c1.1
- Rebuilt for RHEL 6
