...
Version:       2.1.1
Release:       1%{?dist}
Group:         uuzu/Libraries
License:       MIT
URL:           http://www.libgd.org/Main_Page
Source0:       http://www.libgd.org/releases/%{srcname}-%{version}.tar.gz
...
BuildRequires: uuzu-libjpeg-devel, uuzu-libpng-devel, zlib-devel, pkgconfig
...
%package progs
Requires:       uuzu-gd = %{version}-%{release}
...
Requires: uuzu-gd = %{version}-%{release}
Requires: libX11-devel, libXpm-devel, uuzu-libjpeg-devel, freetype-devel
Requires: uuzu-libpng-devel, zlib-devel, fontconfig-devel
Requires: pkgconfig
...
./configure --prefix=%{install_root} \
 ...
 --libdir=%{lib_path} \
 --bindir=%{bin_path} \
 --with-jpeg=%{install_root} \
 --with-png=%{install_root}/libpng \
...
echo '/data/app/lib' > $RPM_BUILD_ROOT/etc/ld.so.conf.d/uuzu-gd.conf
rm $RPM_BUILD_ROOT/%{lib_path}/libgd.la
rm $RPM_BUILD_ROOT/%{lib_path}/libgd.a
...
%defattr(-,root,root,-)
%{lib_path}/*.so.*
/etc/ld.so.conf.d/uuzu-gd.conf

%files progs
%defattr(-,root,root,-)
%{bin_path}/*
%exclude %{bin_path}/gdlib-config

%files devel
%defattr(-,root,root,-)
%{bin_path}/gdlib-config
%{include_path}/*
%{lib_path}/*.so
%{lib_path}/pkgconfig/gdlib.pc
